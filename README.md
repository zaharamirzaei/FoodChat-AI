...
- Interactive Chat UI (via Chainlit)  
  ...
  - Users can exit modules manually with exit, quit, bye, etc.  
...
This will start a local web interface at http://localhost:8000
.
...
module_identifier.py analyzes the query and decides which module should handle it.

The chosen module (food_info, food_services, or food_suggestion) processes the request.

The result is passed back to the main agent (or UI) and returned to the user.
